 ...
 * @license    	https://github.com/scoumbourdis/ci-crud/blob/master/license-ci-crud.txt
 * @version    	1.5.7
 ...
 * @author     	John Skoumbourdis <scoumbourdisj@gmail.com>
 * @license     https://github.com/scoumbourdis/ci-crud/blob/master/license-ci-crud.txt
 * @link		http://www.cicrud.com/documentation
 */
class ci_CRUD_Field_Types
{
	/**
 ...
 * @link		http://www.cicrud.com/documentation
 */
class ci_CRUD_Model_Driver extends ci_CRUD_Field_Types
{
	/**
	...
	protected function form_validation()
	{
		...
		{
			$this->form_validation = new ci_CRUD_Form_validation();
			$ci = &get_instance();
 ...
 * @version    	1.5.7
 */
class ci_CRUD_Layout extends ci_CRUD_Model_Driver
{
	...
	{
		$this->set_js_lib($this->default_javascript_path.'/'.ci_CRUD::JQUERY);
	...
	{
		$this->set_css($this->default_css_path.'/ui/simple/'.ci_CRUD::JQUERY_UI_CSS);
		$this->set_js_lib($this->default_javascript_path.'/jquery_plugins/ui/'.ci_CRUD::JQUERY_UI_JS);
	}
	...
	protected function get_layout()
	{
		...
		if ($this->unset_jquery) {
			unset($js_files[sha1($this->default_javascript_path.'/'.ci_CRUD::JQUERY)]);
		}

		if ($this->unset_jquery_ui) {
			unset($css_files[sha1($this->default_css_path.'/ui/simple/'.ci_CRUD::JQUERY_UI_CSS)]);
			unset($js_files[sha1($this->default_javascript_path.'/jquery_plugins/ui/'.ci_CRUD::JQUERY_UI_JS)]);
		}
	...
	protected function get_upload_file_input($field_info, $value)
	{
		...
		$input .= "<div id='uploader_$unique' rel='$unique' class='ci-crud-uploader' style='$uploader_display_none'></div>";
 ...
 * @version    	1.5.7
 */
class ci_CRUD_States extends ci_CRUD_Layout
{
 ...
 * @link		http://www.cicrud.com/documentation
 */
class Ci_CRUD extends ci_CRUD_States
{
	/**
	...
	protected $default_language_path	= 'assets/ci_crud/languages';
	protected $default_config_path		= 'assets/ci_crud/config';
	protected $default_assets_path		= 'assets/ci_crud';
	...
	{
		$ci = &get_instance();
		$ci->load->config('ci_crud');

		$this->config = (object)array();

		/** Initialize all the config variables into this object */
		$this->config->default_language 	= $ci->config->item('ci_crud_default_language');
		$this->config->date_format 			= $ci->config->item('ci_crud_date_format');
		$this->config->default_per_page		= $ci->config->item('ci_crud_default_per_page');
		$this->config->file_upload_allow_file_types	= $ci->config->item('ci_crud_file_upload_allow_file_types');
		$this->config->file_upload_max_file_size	= $ci->config->item('ci_crud_file_upload_max_file_size');
		$this->config->default_text_editor	= $ci->config->item('ci_crud_default_text_editor');
		$this->config->text_editor_type		= $ci->config->item('ci_crud_text_editor_type');
		$this->config->character_limiter	= $ci->config->item('ci_crud_character_limiter');
		$this->config->dialog_forms			= $ci->config->item('ci_crud_dialog_forms');
		$this->config->paging_options		= $ci->config->item('ci_crud_paging_options');
        $this->config->default_theme        = $ci->config->item('ci_crud_default_theme');
        $this->config->environment          = $ci->config->item('ci_crud_environment');
        $this->config->xss_clean            = $ci->config->item('ci_crud_xss_clean');
	 ...
	 * Without this function nothing works. Here is the core of ci CRUD project.
	 *
	 * @access	public
	 */
	public function render()
	{
		...
		switch ($this->state_code) {
			...
			*/
			...
			case ci_CRUD_States::STATE_READ:
				if($this->unset_read)
				{
					throw new Exception('You don\'t have permissions for this operation', 14);
					die();
            ...
            case ci_CRUD_States::STATE_DELETE_MULTIPLE:
...
if(defined('CI_VERSION'))
{
	...
	class ci_CRUD_Form_validation extends CI_Form_validation{
